...
function App() {
  ...
  async function load() {
    const [dashboard, animalRows, lotRows, supplierRows] =
      await Promise.all(
        ["dashboard", "animals", "lots", "suppliers"].map((path) =>
  ...
  return (
    <div className="app">
      ...
      <main>
        ...
        {page === "history" && <HistoryData animals={animals} />}
      </main>
    </div>
  );
}
  ...
  return (
    <>
      <div className="cards">
        ...
        <Card
          title="Kg actuales en finca"
          value={kg(data.current_weight_kg)}
        />

        <Card
          title="GDP promedio"
          value={gdp(data.average_gdp_kg_day)}
        />
      </div>
      ...
      <div className="panel">
        ...
        <p className="muted">
          El sistema ya conserva historial de peso por animal y calcula
          ganancia acumulada, días en finca y GDP.
        </p>
      ...
      <div className="cards secondary-cards">
        <Card
          title="Peso inicial total"
          value={kg(data.initial_weight_kg)}
        />

        <Card
          title="Ganancia acumulada"
          value={kg(data.total_gain_kg)}
        />

        <Card
          title="Peso promedio compra"
          value={kg(data.average_initial_weight_kg)}
        />

        <Card
          title="Peso promedio actual"
          value={kg(data.average_current_weight_kg)}
        />
      </div>
    </>
  );
...
function Purchase({ suppliers, onDone }) {
  ...
  async function save() {
    ...
    try {
      ...
      onDone(
        `Compra ${result.lot_code} registrada: ${result.animals} animales, ${kg(
          result.total_weight_kg
        )}, ${money(result.investment)} invertidos.`
      );
    ...
    <div className="panel">
      <div className="form-head">
        <div>
          ...
          <p className="muted">
            Registra la operación completa y crea el lote automáticamente.
          </p>
        </div>
      ...
      <div className="grid">
        <label>
          ...
          <input
            ...
            onChange={(event) =>
              update("entry_date", event.target.value)
            }
          />
        </label>
        ...
        <label>
          ...
          <input
            ...
            value={form.lot_code}
            onChange={(event) =>
              update("lot_code", event.target.value)
            }
          />
        </label>
        ...
        <label>
          ...
          <select
            ...
            onChange={(event) =>
              update("supplier_id", event.target.value)
            }
          >
        ...
        <label>
          ...
          <input
            ...
            onChange={(event) =>
              update("target_weight_kg", event.target.value)
            }
            placeholder="Ej. 400"
          />
        </label>
...
function AnimalSearchSelect({
  ...
  function pickAnimal() {
    const exactMatch = filteredAnimals.find(
      (animal) =>
        animal.ear_tag.toLowerCase() ===
        search.trim().toLowerCase()
    );

    const animal =
      filteredAnimals.find(
    ...
    if (!animal) {
      alert(
        "Digita y selecciona una coincidencia de la lista."
      );
  ...
  return (
    <div className="searchable-picker">
      ...
      <select
        size={Math.min(
          Math.max(filteredAnimals.length, 2),
          6
        )}
        value={selectedId}
        onChange={(event) =>
        ...
        {filteredAnimals.length ? (
          filteredAnimals.map((animal) => (
            <option value={animal.id} key={animal.id}>
              {animal.ear_tag} | {animal.lot || "Sin lote"} |
              actual {kg(animal.current_weight_kg)}
            </option>
          ))
        ) : (
...
function Weighings({ animals, onDone }) {
  const activeAnimals = animals.filter(
    (animal) => animal.status === "Activo"
  );
      ...
      <div className="panel">
        <div className="form-head">
          <div>
            ...
            <p className="muted">
              Registra manualmente varios animales en una
              misma fecha.
            </p>
          </div>
              ...
              {rows.length ? (
                ...
                <tr>
                  <td
                    ...
                    className="empty"
                  >
                    Selecciona animales para comenzar.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="summary">
          <b>{pendingSummary.count} con peso</b>

          <span>
            Total jornada:{" "}
            {kg(pendingSummary.total)}
          </span>
        </div>
      </div>
    </div>
...
function HistoryData({ animals }) {
  const [history, setHistory] = useState(null);

  async function viewAnimal(animal) {
    try {
      const response = await fetch(
        `${API}/api/weighings/animal/${animal.id}`
      );
  ...
  return (
    <div className="history-layout">
      <div className="panel">
        <h2>Historial y Datos</h2>

        <p className="muted">
          Consulta la información productiva y el
      ...
      {history && (
        <>
          <div className="panel">
            <div className="animal-header">
              <div>
                ...
                <h2>
                  {history.animal.ear_tag}
                </h2>

                <p className="muted">
                  Entrada:{" "}
                  {history.animal.entry_date}{" "}
                  · Estado:{" "}
                  {history.animal.status}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="panel">
            ...
            <p className="muted">
              Próximamente: vacunas,
              desparasitaciones, baños,
              tratamientos y otros eventos.
            </p>
          </div>
          ...
          <div className="panel">
            ...
            <p className="muted">
              Próximamente: costos individuales,
              costos asignados al lote y costo
              acumulado.
            </p>
          </div>
        </>
    ...
    <div className="panel">
      <div className="form-head">
        <div>
          ...
          <p className="muted">
            Cada novillo conserva su identificación,
            lote, peso inicial, peso actual y GDP.
          </p>
        </div>
      </div>
